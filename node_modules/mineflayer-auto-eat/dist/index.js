...
export function plugin(bot) {
    ...
    bot.autoEat.eat = async (useOffhand = bot.autoEat.options.offhand) => {
        if (bot.autoEat.disabled || bot.autoEat.isEating || bot.food > 19)
            return false;
        let startAt = bot.autoEat.options.startAt;
        if (bot.autoEat.options.priority === 'auto' &&
        ...
        const bestChoices = items
            ...
            .sort((a, b) => {
            if (priority !== 'auto')
                return food[b.name][priority] - food[a.name][priority];
